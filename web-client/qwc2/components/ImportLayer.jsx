...
class ImportLayer extends React.Component {
  ...
  renderInputField() {
    ...
    if (this.state.type === "Local") {
      return (
        <FileSelector accept=".kmz,.kml,.json,.geojson" file={this.state.file} onFileSelected={this.onFileSelected} />
      );
    } else {
      return (
        <EditableSelect
  ...
  render() {
    ...
    if (this.state.serviceLayers !== null) {
      layerList = (
        <LayerCatalogWidget catalog={this.state.serviceLayers} pendingRequests={this.state.pendingRequests} />
      );
    }
      ...
      <div id="ImportLayer">
        <div className="importlayer-input-fields">
          <select
            ...
            onChange={(ev) => this.setState({ type: ev.target.value, file: null, url: "", serviceLayers: null })}
  ...
  scanService = () => {
    ...
    if (
      reqUrl.toLowerCase().endsWith(".json") ||
      (reqUrl.toLowerCase().endsWith(".xml") && !reqUrl.toLowerCase().endsWith("wmtscapabilities.xml"))
    ) {
          ...
          if (type === "xml") {
            ...
            for (const service of ["wms", "wfs"]) {
              ...
              for (const conn of [].slice.call(connections[0].getElementsByTagName(service))) {
                ...
                connUrl +=
                  (connUrl.includes("?") ? "&" : "?") + "service=" + service.toUpperCase() + "&request=GetCapabilities";
                this.setState({ pendingRequests: ++pendingRequests });
                axios
                  .get(connUrl)
                  .then((connResponse) => {
                    const result =
                      service === "wms"
                        ? ServiceLayerUtils.getWMSLayers(connResponse.data, connUrl, true)
                        : ServiceLayerUtils.getWFSLayers(connResponse.data, connUrl, this.props.mapCrs);
                    this.setState({
                      pendingRequests: this.state.pendingRequests - 1,
    ...
    return jszip.loadAsync(kmzFile).then((zip) => {
      ...
      zip.forEach((relPath, file) => {
        if (getExtension(relPath) === "kml" && kml === null) {
          kml = file.async("string").then(getDom);
        }
      });
