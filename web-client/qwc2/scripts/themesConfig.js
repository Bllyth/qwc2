...
function getThumbnail(configItem, resultItem, layers, crs, extent, resolve, proxy) {
  ...
  const adjustedExtent = [bboxcx - 0.5 * bboxw, bboxcy - 0.5 * bboxh, bboxcx + 0.5 * bboxw, bboxcy + 0.5 * bboxh];
...
function getTheme(config, configItem, result, resultItem, proxy) {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get(getCapabilitiesUrl, { proxy: proxy, auth: configItem.wmsBasicAuth })
      .then((response) => {
        // parse capabilities
        let capabilities;
        xml2js.parseString(
          ...
          for (const composerTemplate of templates) {
            if (composerTemplate.ComposerMap !== undefined) {
              ...
              if (composerTemplate.ComposerLabel !== undefined) {
                printTemplate.labels = toArray(composerTemplate.ComposerLabel)
                  .map((entry) => {
                    return entry.$.name;
                  })
        ...
        // drawing order
        const drawingOrder = (capabilities.Capability.LayerDrawingOrder || "")
          .split(",")
          .map((title) => (title in titleNameMap ? titleNameMap[title] : title));
        ...
        if (configItem.legendUrl) {
          ...
          resultItem.legendUrl =
            capabilities.Capability.Request.GetLegendGraphic.DCPType.HTTP.Get.OnlineResource.$["xlink:href"].replace(
              /\?$/,
              ""
            ) +
            "?" +
            (configItem.extraLegendParameters ? configItem.extraLegendParameters : "");
        }
        if (configItem.featureInfoUrl) {
          resultItem.featureInfoUrl = configItem.featureInfoUrl;
        } else {
          resultItem.featureInfoUrl =
            capabilities.Capability.Request.GetFeatureInfo.DCPType.HTTP.Get.OnlineResource.$["xlink:href"].replace(
              /\?$/,
              ""
            ) + "?";
        }
        if (configItem.printUrl) {
          ...
          resultItem.printUrl =
            capabilities.Capability.Request.GetPrint.DCPType.HTTP.Get.OnlineResource.$["xlink:href"].replace(
              /\?$/,
              ""
            ) + "?";
        }
